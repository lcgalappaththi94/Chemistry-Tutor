...
public class QuestionTopicMapper implements RowMapper<Question> {
    ...
    public Question mapRow(ResultSet rs, int i) throws SQLException {
        Question ques = new Question("" + rs.getInt("qNo"), "" + rs.getInt("authorId"), rs.getString("topic"), rs.getString("question"), rs.getString("media"), rs.getString("corAnwrId"), rs.getString("difficulty"));
        return ques;
    }
}
